...
| "换个目录工作"、"移动会话到那个项目" | **`/cd <path>`** | 移动会话到新目录，保留对话缓存 |
...
| "多想想"、"认真点"、"给多点算力" | **`/effort high`** | 调节推理深度；`ultracode` = xhigh + auto workflow |
| "让另一个模型给建议"、"开启顾问" | **`/advisor opus`**（或 `sonnet`/`fable`） | 启用顾问模型，关键时刻提供第二意见 |
...
| "安装插件"、"列出已安装插件" | **`/plugin`** | 插件管理；`/plugin list` 内联列出 |
| "直接改设置"、"关闭 thinking" | **`/config key=value`** | 如 `/config thinking=false`，不打开界面直接设 |
...
```
会话    /clear /compact /context /resume /branch /fork /background /stop /rename /cd /rewind /recap /export /copy
模型    /model /effort /advisor /fast /plan /ultraplan /goal /exit
